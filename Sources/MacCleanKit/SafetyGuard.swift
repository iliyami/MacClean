    ...
    public enum SafetyError: Error, LocalizedError, Equatable {
        ...
        public var errorDescription: String? {
            ...
            case .protectedPath(let path):
                L10n.tr("无法修改受保护的系统路径：\(path)", "Cannot modify protected system path: \(path)", "Невозможно изменить защищённый системный путь: \(path)")
            case .tooManyFiles(let count):
                L10n.tr("操作超过 \(MCConstants.maxFilesPerOperation) 个文件的安全限制（尝试处理：\(count) 个）", "Operation exceeds safety limit of \(MCConstants.maxFilesPerOperation) files (attempted: \(count))", "Операция превышает безопасный лимит: \(MCConstants.maxFilesPerOperation) файлов (запрошено: \(count))")
            case .symlinkTarget(let path):
                L10n.tr("路径通过符号链接指向了意外位置：\(path)", "Path resolves through symlink to unexpected location: \(path)", "Путь через символическую ссылку ведёт в непредусмотренное расположение: \(path)")
            case .sipProtected(let path):
                L10n.tr("路径受系统完整性保护（SIP）保护：\(path)", "Path is protected by System Integrity Protection: \(path)", "Путь защищён функцией System Integrity Protection (SIP): \(path)")
