...
**支持英文、简体中文和俄文（Русский）。** 可随时在 **设置 → 界面语言** 中切换；默认跟随系统语言。
...
- **可配置的菜单栏读数**：可选择显示可用磁盘空间、GPU 使用率、内存使用率或电池温度；选择会自动保存，不可用的传感器显示 `--`
...
- **可取消的扫描**：智能扫描和重复文件在扫描中显示「取消」按钮；`ScanCoordinator.cancel()` 约在一秒内将界面返回空闲状态，不留下孤儿任务
- **重复文件合并**（不删除即可回收空间）：在 APFS 上，多余副本会被替换为保留主副本的写时复制克隆，因此每个路径都能继续使用，相同文件不再占用 N 倍空间
